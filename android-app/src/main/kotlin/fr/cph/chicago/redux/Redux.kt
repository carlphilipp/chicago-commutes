...
fun reducer(action: Action, oldState: State?): State {
    ...
    when (action) {
        ...
        is ResetBusRoutesFavoritesAction -> {
            state = state.copy(
                busRoutesStatus = Status.UNKNOWN,
                random = UUID.randomUUID().toString()
            )
        }
        ...
        is ResetTrainStationStatusAction -> {
            state = state.copy(
                trainStationStatus = Status.UNKNOWN,
                random = UUID.randomUUID().toString()
            )
        }
        is ResetBusStationStatusAction -> {
            state = state.copy(
                busStopStatus = Status.UNKNOWN,
                random = UUID.randomUUID().toString()
            )
        }
        is ResetBikeStationStatusAction -> {
            state = state.copy(
                bikeStationsStatus = Status.UNKNOWN,
                random = UUID.randomUUID().toString()
            )
        }
        is ResetAlertsStatusAction -> {
            state = state.copy(
