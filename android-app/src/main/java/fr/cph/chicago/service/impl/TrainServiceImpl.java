...
public enum TrainServiceImpl implements TrainService {
        ...
        try {
            for (final Map.Entry<String, Collection<String>> entry : trainParams.asMap().entrySet()) {
                ...
                if ("mapid".equals(key)) {
                    ...
                    if (list.size() < 5) {
                        final InputStream xmlResult = CtaClient.INSTANCE.connect(TRAIN_ARRIVALS, trainParams);
                        ...
                        while (end < size + 1) {
                            ...
                            final InputStream xmlResult = CtaClient.INSTANCE.connect(TRAIN_ARRIVALS, paramsTemp);
    ...
    public Optional<TrainArrival> loadStationTrainArrival(@NonNull final Context context, int stationId) {
        try {
            ...
            final InputStream xmlResult = CtaClient.INSTANCE.connect(TRAIN_ARRIVALS, params);
